...
function htmlReport(data: ReportData, generatedAt: string): string {
  ...
  const findings = report.findings
    .map(
      (finding) =>
        `<tr><td>${escapeHtml(finding.severity)}</td><td>${escapeHtml(finding.status)}</td><td>${escapeHtml(finding.ruleName)}</td><td>${escapeHtml(finding.kind)}</td><td>${escapeHtml(finding.maskedValue)}</td><td>${escapeHtml(finding.assetUrl)}</td><td>${finding.line}</td><td>${escapeHtml(`${finding.reviewNote}\n${finding.preview}`)}</td></tr>`,
    )
    .join("");
